<script lang="ts">
  ...
  // calendar sidebar. Replaces the older single-purpose "+ Add CalDAV"
  // button. Click expands the button into four stacked source options
  // (Local / CalDAV / Google / Microsoft), each with a small fly-in
  // stagger. Click an option to open its add flow; click the chevron
  // again (or anywhere outside the menu) to collapse.
  //
  // Phase 2 entries (Google / Microsoft) render visually but are
  // disabled — they show their icons + labels at reduced opacity with
  // cursor-not-allowed. The slots exist now so the layout doesn't shift
  // when Phase 2 wires them up.
  //
  // Owns its own dialog mounts (AddLocalCalendarDialog,
  // AddCalDAVSourceDialog) so the consumer doesn't have to coordinate
  // dialog state.
...
<div class="px-3 py-2" bind:this={containerRef}>
  ...
  {#if expanded}
    <div class="mt-1 space-y-0.5">
      ...
      <!-- Google (Phase 2, disabled) -->
      <button
        type="button"
        disabled
        aria-disabled="true"
        class="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground/50
               rounded-md cursor-not-allowed select-none"
        transition:fly={{ y: -8, duration: 180, delay: 80, easing: cubicOut }}
      >
        <Icon icon="logos:google-icon" class="w-4 h-4 opacity-50" />
        <span class="flex-1 text-left">{$_('calendar.addCalendar.google')}</span>
      </button>

      <!-- Microsoft (Phase 2, disabled) -->
      <button
        type="button"
        disabled
        aria-disabled="true"
      ...
      >
        <Icon icon="logos:microsoft-icon" class="w-4 h-4 opacity-50" />
